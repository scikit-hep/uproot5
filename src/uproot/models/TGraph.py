...
def as_TGraph(
    ...
    for x, y in zip(fX, fY):
        if not (isinstance(x, (int, float)) and isinstance(y, (int, float))):
            raise ValueError("Values in the arrays have to be numbers!")
    
    if fMinimum is None or fMaximum is None:
        min_val_y = min(fY)
        max_val_y = max(fY)
    ...
    tGraph._members["fMinimum"] = fMinimum if fMinimum is not None else min_val_y 
    tGraph._members["fMaximum"] = fMaximum if fMaximum is not None else max_val_y 
