...
def _float16_double32_walk_ast(node, branch, source):
    if isinstance(node, ast.AST):
        if (
            ...
            raise UnknownInterpretation(
                "cannot compute streamer title {}".format(repr(source)),
                branch.file.file_path,
                branch.object_path,
            )
...
def _float16_or_double32(branch, context, leaf, is_float16, dims):
    ...
    try:
        ...
        try:
            ...
            raise UnknownInterpretation(
                "cannot parse streamer title {} (as Python)".format(repr(source)),
                branch.file.file_path,
                branch.object_path,
            )
...
class UnknownInterpretation(Exception):
    ...
    def __repr__(self):
        return "<UnknownInterpretation {}>".format(repr(self.reason))
