...
class Model_TStreamerInfo(uproot.model.Model):
    ...
    def class_code(self):
        ...
        return "\n".join(
            [f"class {class_name}(uproot.model.VersionedModel):"]
            + read_members
            + read_member_n
            + strided_interpretation
            + awkward_form
...
class Model_TStreamerBasicPointer(Model_TStreamerElement):
    ...
    def class_code(
        ...
        read_members.append("        tmp = self._dtype{}".format(len(dtypes)))
        read_member_n.append("    " + read_members[-1])
                ...
                if len(fields[-1]) == 1:
                    ...
                    read_members.append(
                        "        {} = cursor.fields(chunk, self._format{}, context)".format(
                            ", ".join(
                                "self._members[{}]".format(repr(x)) for x in fields[-1]
                            ),
                            len(formats) - 1,
                        )
                    )
...
class Model_TStreamerLoop(Model_TStreamerElement):
    ...
    def class_code(
        ...
        read_members.extend(
            [
                "        cursor.skip(6)",
                "        for tmp in uproot._util.range(self.member({})):".format(
