...
char* _uproot_TMessage_reallocate(char* buffer, size_t newsize, size_t oldsize) {
    std::any pyptr;
    TPython::Exec("_anyresult = __import__('uproot').pyroot.pyroot_to_buffer.sizer_asany", &pyptr);
    ...
    std::any out;
    TPython::Exec("_anyresult = __import__('uproot').pyroot.pyroot_to_buffer.buffer_ptr_asany", &out);
    return reinterpret_cast<char*>(std::any_cast<size_t>(out));
}
