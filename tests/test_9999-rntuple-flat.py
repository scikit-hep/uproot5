...
def test():
    ...
    with uproot.open(filename) as f:
        ...
        assert all(R.arrays(entry_start=1, entry_stop=3)["one_integers"] == numpy.array([8, 7]))

    filename = skhep_testdata.data_path("test_ntuple_int_5e4.root")
    with uproot.open(filename) as f:
        R = f["ntuple"]
        assert all(R.arrays(entry_stop=3)["one_integers"] == numpy.array([50000, 49999, 49998]))
