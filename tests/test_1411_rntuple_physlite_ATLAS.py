...
def physlite_file():
    ...
    with uproot.open(file_path) as f:
        ...
        assert f["EventData"].classname == "ROOT::RNTuple", "EventData is not an RNTuple"
...
def test_analysis_muons_kinematics(physlite_file):
    ...
    for i, (col, arr) in enumerate(arrays.items()):
        ...
        assert len(ak.flatten(arr)) == n_expected_muons, f"{col} does not match expected muon count"
        assert round(ak.mean(arr),2) == expected_means[i], f"{col} mean value does not match the expected one"
...
def test_event_info(physlite_file):
    ...
    for i, (col, value) in enumerate(first_event.items()):
        assert value == expected_values[i], f"First event {col} doest not match the expected value"
...
def test_truth_muon_containers(physlite_file):
    ...
    assert arrays["TruthMuons"].fields==AOD_type, f"TruthMuons fields have changed, {arrays['TruthMuons'].fields} instead of {AOD_type}"
