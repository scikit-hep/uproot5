...
def test_dask_concatenation():
    ...
    dask_arrays = uproot.dask([test_path1, test_path2, test_path3, test_path4],library='np')
...
def test_chunking_multiple_num():
    ...
    assert uproot.dask([filename1, filename2], step_size=30,library='np')["px1"].chunks == (
        (
            30,
            30,
            30,
...
def test_chunking_multiple_string():
    filename1 = skhep_testdata.data_path("uproot-Zmumu.root") + ":events"
    filename2 = skhep_testdata.data_path("uproot-Zmumu-uncompressed.root") + ":events"
    assert uproot.dask([filename1, filename2], step_size="100B",library='np')["px1"].chunks == (
        (
            19,
            19,
            19,
