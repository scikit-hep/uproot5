...
def test_single_dask_array():
    ...
    assert list(dask_arrays.keys()) == list(np_arrays.keys()), "Different keys detected in dictionary of dask arrays and dictionary of numpy arrays"

    for key in np_arrays.keys():
        comp = dask_arrays[key].compute() == np_arrays[key]
        assert comp.all(), "Incorrect array at key {}".format(key)
...
def test_dask_concatenation():
    ...
    np_arrays = uproot.concatenate([test_path1,test_path2,test_path3,test_path4],library='np')
...
def test_multidim_array():
    test_path = skhep_testdata.data_path("uproot-sample-6.08.04-uncompressed.root") + ":sample"
    ttree = uproot.open(test_path)
