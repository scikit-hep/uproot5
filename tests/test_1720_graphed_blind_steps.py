...
def test_blind_partitions_do_not_open_the_file():
    ...
    assert all(
        t.partition.entry_start == 0 and t.partition.entry_stop == 0 for t in tasks
    )
...
def test_blind_and_eager_partitions_agree():
    ...
    test_path = skhep_testdata.data_path("uproot-Zmumu.root") + ":events"
    blind = _run(
        uproot.graphed_partitions(test_path, steps_per_file=7, open_files=False)
    )
    eager = _run(
        uproot.graphed_partitions(test_path, steps_per_file=7, open_files=True)
    )
