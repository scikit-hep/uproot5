...
def test_new_support_RNTuple_split_int16_reading(backend, GDS, library):
    with uproot.open(
        ...
        assert ak.all(library.unique(df.one_integers[: len(df.one_integers) // 2]) == library.array([2]))
        assert ak.all(library.unique(df.one_integers[len(df.one_integers) / 2 + 1 :]) == library.array([1]))
