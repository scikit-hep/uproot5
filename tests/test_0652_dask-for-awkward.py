...
def test_dask_concatenation():
    ...
    ak_array = uproot.concatenate(
        [test_path1, test_path2, test_path3, test_path4]
    )
    dak_array = uproot.dask([test_path1, test_path2, test_path3, test_path4],library='ak')
...
def test_chunking_multiple_num():
    ...
    assert uproot.dask([filename1,filename2],step_size=10,library='ak').npartitions == 462

def test_chunking_multiple_string():
    filename1 = skhep_testdata.data_path("uproot-Zmumu.root") + ":events"
    filename2 = skhep_testdata.data_path("uproot-Zmumu-uncompressed.root") + ":events"
    assert uproot.dask([filename1,filename2],step_size="500B",library='ak').npartitions == 1098
...
def test_delay_open():
    ...
    dak_array = uproot.dask([test_path1, test_path2, test_path3, test_path4],open_files=False,library='ak')
