...
def test_hard_out_of_bounds_returns_empty(tree):
    ...
    data = tree["Muon_Px"].array(
        entry_start=total + 100, entry_stop=total + 200, library="np"
    )
    assert len(data) == 0
...
def test_clamped_out_of_bounds_truncates(tree):
    ...
    data = tree["Muon_Px"].array(
        entry_start=total - 5, entry_stop=total + 100, library="ak"
    )
    assert len(data) == 5
...
def test_exact_boundary_start(tree):
    ...
    data = tree["NJet"].array(entry_start=total, entry_stop=total + 10, library="np")
    assert len(data) == 0
