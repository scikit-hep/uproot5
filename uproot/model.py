...
""".format(
    ...
    @classmethod
    def read_memberwise(cls, chunk, cursor, context, file, selffile, parent, concrete=None, reuse_model=None):
        """
            reuse_model (None or :doc:`uproot.model.Model` instance): If None,
                then continue to create new models and read all members
                contiguously. Otherwise, reuse the model and read out memberwise.
        """
        self = cls.__new__(cls)
        self._cursor = cursor.copy()
        self._file = selffile
...
""".format(
        ...
        # versioned_cls.read starts with numbytes_version again because move=False (above)
        return cls.postprocess(
            versioned_cls.read_memberwise(
                chunk, cursor, context, file, selffile, parent, concrete=concrete, reuse_model=reuse_model
            ),
            chunk,
            cursor,
            context,
